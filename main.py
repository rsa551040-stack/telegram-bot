...
async def send_alert(update: Update, text: str) -> None:
    ...
    try:
        ...
        if ALERT_MESSAGE_AUTO_DELETE_SECONDS > 0:
            # حذف رسالة التنبيه لاحقًا لو أمكن
            ...
            import asyncio
            asyncio.create_task(_sleep_and_delete(ALERT_MESSAGE_AUTO_DELETE_SECONDS, _delete_alert))
    except Exception as exc:
        logger.warning("Failed to send alert: %s", exc)


async def _sleep_and_delete(seconds: int, callback) -> None:
    import asyncio
...
async def moderate_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if should_ban_for_ad:
        ...
        if banned:
            await send_alert(
                update,
                f"🚫 تم حذف رسالة إعلانية وحظر المستخدم {user.mention_html()}.",
            )
        else:
            await send_alert(
                update,
                f"⚠️ تم حذف رسالة مخالفة من {user.mention_html()} لكن فشل الحظر. تأكد من صلاحيات البوت.",
            )
        return
    ...
    if ABUSE_BAN_THRESHOLD > 0 and strikes >= ABUSE_BAN_THRESHOLD:
        ...
        if banned:
            await send_alert(
                update,
                f"🚫 تم حظر المستخدم {user.mention_html()} بعد تجاوز حد الكلمات المسيئة.",
            )
        else:
            await send_alert(
                update,
                f"⚠️ تم حذف رسالة مسيئة من {user.mention_html()} لكن فشل الحظر. تأكد من صلاحيات البوت.",
            )
    else:
        await send_alert(
            update,
            f"🗑️ تم حذف رسالة مسيئة من {user.mention_html()}.",
        )
...
def main() -> None:
    if not BOT_TOKEN:
        raise RuntimeError("BOT_TOKEN is missing. Please set it in .env or environment variables.")
